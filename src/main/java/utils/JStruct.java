...
public class JStruct {
    ...
    public byte[] pack(String fmt, long val) throws Exception{
        if(fmt.length()>2){
            throw new RuntimeException("Single values may not have multiple format specifiers");
        }
        ...
        for (int i=0; i<fmt.length(); i++){
            ...
            if ((i == 0) && ((c == '>') || (c == '<') || (c == '@') || (c == '!'))){
                ...
                if (bx == null)
                    throw new RuntimeException("Invalid character specifier: "+ c);
    ...
    public byte[] pack(String fmt, long ... vals ) throws Exception{
        ...
        if(len!=vals.length)
            throw new RuntimeException("format length and values aren't equal");
